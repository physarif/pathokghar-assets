"""
Shared utilities used by generate_pdf.py, generate_epub.py, and
generate_mobi.py: Firebase fetching, file downloads, HTML cleanup shared by
all three formats, epub building (used directly by generate_epub.py and as
an intermediate step by generate_mobi.py), and the common per-book driver
loop (run()) that each generate_*.py plugs its own conversion step into.
"""
import argparse
import json
import os
...
def run(fmt, ext, build_one, setup=None, test_limit=5):
    """The per-book driver shared by all three generate_*.py scripts.

    fmt: format name used for the output folder + css file ("epub"/"pdf"/"mobi")
    ext: output file extension (usually same as fmt)
    build_one(book, html_files, cover_path, work_dir, extract_dir, out_dir,
              css_path, ctx) -> out_path: the one format-specific step each
              generate_*.py plugs in.
    setup(work_root) -> ctx dict: optional one-time setup (e.g. downloading
              a shared banner image for PDF) run once before the per-book
              loop; its return value is passed as `ctx` to build_one.
    test_limit: cap on how many pending books to build per run. Set to
              None to build everything.
    """
    ...
    ctx = setup(WORK) if setup else {}

    pending = list_pending_books(ext, existing_names)

    if test_limit is not None and len(pending) > test_limit:
    ...
    for b in pending:
        ...
        try:
            ...
            print(f"[{slug}] 1/4: downloading zip...", flush=True)
            download(b["zip_url"], zip_path)

            print(f"[{slug}] 2/4: extracting html...", flush=True)
            ...
            cover_path = None
            if b.get("cover_url"):
                print(f"[{slug}] 3/4: downloading cover...", flush=True)
                tmp_cover = os.path.join(work_dir, "cover_download")
                try:
                    content_type = download(b["cover_url"], tmp_cover)
                    ext_ = guess_image_ext(tmp_cover, b["cover_url"], content_type)
                    cover_path = os.path.join(work_dir, f"cover{ext_}")
                    os.replace(tmp_cover, cover_path)
                except Exception as e:
                    print(f"[{slug}] cover download failed: {e}")
                    cover_path = None
            else:
                print(f"[{slug}] 3/4: (no cover)", flush=True)

            print(f"[{slug}] 4/4: building {fmt}...", flush=True)
            out_path = build_one(b, html_files, cover_path, work_dir, extract_dir, out_dir, css_path, ctx)
